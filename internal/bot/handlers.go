...
func (b *Bot) handleMessage(ctx context.Context, m *tg.Message) {
	if m.Voice != nil || m.Audio != nil {
		v := m.Voice
		if v == nil {
			v = m.Audio
		}
		b.handleVoice(ctx, m, v)
		return
	}
	...
	if !strings.HasPrefix(text, "/") {
		b.handleNote(ctx, m.Chat.ID, text)
		return
	}
	...
	case "d":
		b.handleDay(ctx, m.Chat.ID, args)
	case "m":
		b.handleMoney(ctx, m.Chat.ID, args)
	case "s":
		b.handleStatus(ctx, m.Chat.ID)
	case "w":
		b.handleWeekly(ctx, m.Chat.ID, args)
	case "undo":
		b.handleUndo(ctx, m.Chat.ID)
	default:
...
func (b *Bot) handleNote(ctx context.Context, chatID int64, text string) {
...
func (b *Bot) handleDay(ctx context.Context, chatID int64, args string) {
...
func (b *Bot) handleMoney(ctx context.Context, chatID int64, args string) {
	...
	if m.Kind == model.MoneySaving {
		if all, err := b.st.MoneyUntil(b.cfg.Today()); err == nil {
			var capital float64
			for _, x := range all {
...
func (b *Bot) handleStatus(ctx context.Context, chatID int64) {
	today := b.cfg.Today()
	day, err := b.st.GetDay(today)
	if err != nil {
		b.reply(ctx, chatID, "Не смог прочитать запись: "+err.Error())
		return
	}
	st, err := b.stats(report.AddDays(today, -6), today)
	if err != nil {
...
func (b *Bot) handleWeekly(ctx context.Context, chatID int64, args string) {
	...
	if err := b.sendReport(ctx, chatID, n); err != nil {
...
func (b *Bot) sendReport(ctx context.Context, chatID int64, n int) error {
	to := b.cfg.Today()
	from := report.AddDays(to, -(n - 1))
	st, err := b.stats(from, to)
	if err != nil {
		return err
	}
...
func (b *Bot) handleUndo(ctx context.Context, chatID int64) {
	u, err := b.st.LastUndoable()
	if err != nil {
	...
	if err := b.st.Delete(u.Kind, u.ID); err != nil {
...
func (b *Bot) RemindDay(ctx context.Context) error {
	date := b.cfg.Today()
	day, err := b.st.GetDay(date)
	if err != nil {
		return err
	}
	missing := day.MissingFields()
	if len(missing) == 0 {
		return b.Notify(ctx, "🌙 День закрыт полностью. Ничего не жду.")
	}
	names := make([]string, 0, len(missing))
	for _, f := range missing {
		names = append(names, f.Keys[0])
...
func (b *Bot) SendWeekly(ctx context.Context) error {
	return b.sendReport(ctx, b.cfg.OwnerID, b.cfg.WeeklyReportN)
}
